...
PureAdmin.portlets = {
  ...
  loadPortlet: function(portlet, url) {
    ...
    if (portlet.data('body-loaded') || portlet.hasClass('loading') || !source) { return; }
    ...
    if (loading !== false) {
      if (!PureAdmin.portlets.loadingTimer[uniqueId]) {
        PureAdmin.portlets.loadingTimer[uniqueId] = setTimeout(function() {
          ...
          var elem_body = elem.find('.portlet-body');
          elem_body.addClass('loading');
          $(elem_body).prepend('<div class="overlay" style="width:'+
            $(elem_body).width() +'px; height:'+ '200px;"></div>');
        }, 300);
      }
    } else {
      ...
      var elem_body = elem.find('.portlet-body');
      elem_body.removeClass('loading');
      $(elem_body).children('div.overlay').remove();
    }
  },
  ...
  errorResponse: function(thrown) {
    Flash.create('error', 'An error occured when loading the remote URL.');
